...
class Santa
  def nice?(string)
    has_three_vowels?(string) && has_repeated_letter?(string) && !has_forbidden_strings?(string)
  end
  ...
  def new_nice?(string)
    has_repeated_pair?(string) && has_xyx?(string)
  end
...
if defined? DATA
  input = DATA.read
  puts Santa.new.num_nice(input)
  puts Santa.new.num_new_nice(input)
end
