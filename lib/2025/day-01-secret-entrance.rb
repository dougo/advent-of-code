...
open the safe. A rotation starts with an L or R which indicates whether the rotation should be to the left (toward
lower numbers) or to the right (toward higher numbers). Then, the rotation has a distance value which indicates how
...
class Safe
  ...
  def initialize(text)
    @dial_rotations = text.lines.map { DialRotation.new(_1) }
  end

  class DialRotation
    attr :direction, :distance
