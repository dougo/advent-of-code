...
class AuntSue
  ...
  def match?(ticker)
    @counts.all? { |name, count| ticker[name] == count }
  end

  def match_retro_encabulator?(ticker)
    @counts.all? do |name, count|
      case name
      when :cats, :trees
        count > ticker[name]
  ...
  class List < Array
    def find_match(ticker, matcher = :match?)
      find { |sue| sue.send(matcher, ticker) }.number
    end

    def find_match_retro_encabulator(ticker)
      find_match(ticker, :match_retro_encabulator?)
    end
  end
end
...
if defined? DATA
  ...
  sues = AuntSue.parse_list(DATA.read.chomp)
  puts sues.find_match(ticker)
  puts sues.find_match_retro_encabulator(ticker)
end
