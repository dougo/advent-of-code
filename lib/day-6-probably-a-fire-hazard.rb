...
class Lights
  ...
  def do_light(command, row, col)
    light = @lit[row][col]
    case command
    when 'turn on'
      @lit[row][col] = turn_on(light)
    when 'toggle'
      @lit[row][col] = toggle(light)
    when 'turn off'
      @lit[row][col] = turn_off(light)
    end
  end
...
if defined? DATA
  input = DATA.read
  puts Lights.new.do_instructions(input).num_lit
  puts CorrectedLights.new.do_instructions(input).brightness
end
