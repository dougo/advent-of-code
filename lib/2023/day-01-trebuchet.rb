...
class CalibrationDocument
  def initialize(text)
    @lines = text.lines.map { line(_1) }
  end

  def line(text) = Line.new(text)
  ...
  class Line
    ...
    def first_digit
      digit_at(text.index(/\d/))
    end

    def last_digit
      digit_at(text.rindex(/\d/))
    end
...
class FixedCalibrationDocument < CalibrationDocument
  def line(text) = FixedLine.new(text)
...
if defined? DATA
  input = DATA.read
  puts CalibrationDocument.new(input).sum
  puts FixedCalibrationDocument.new(input).sum
end
