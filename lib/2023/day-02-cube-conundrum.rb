class CubeGame
  class List
    def initialize(input)
      @games = input.lines.map { CubeGame.new(_1) }
    end

    attr :games

    def sum_of_possible_game_ids(bag)
      possible_games = games.filter { _1.possible?(bag) }
      possible_games.sum &:id
    end

    def minimum_sets_of_cubes
      games.map &:minimum_set_of_cubes
    end

    def powers_of_minimum_sets_of_cubes
      minimum_sets_of_cubes.map { CubeGame.power(_1) }
    end

    def sum_of_powers_of_minimum_sets_of_cubes
      powers_of_minimum_sets_of_cubes.sum
    end
  end

  def self.power(cube_set)
    cube_set.values.reduce(&:*)
  end
  ...
  def minimum_set_of_cubes
    %i(red green blue).map do |color|
      [color, reveals.map { _1.amount_revealed(color) }.max]
    end.to_h
  end
  ...
  class Reveal
    ...
    def amount_revealed(color)
      @revealed.fetch(color, 0)
    end
  end
end

if defined? DATA
  games = CubeGame::List.new(DATA.read)
  puts games.sum_of_possible_game_ids(red: 12, green: 13, blue: 14)
  puts games.sum_of_powers_of_minimum_sets_of_cubes
end
