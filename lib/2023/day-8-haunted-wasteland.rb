...
class HauntedWasteland
  ...
  def steps_required
    @left_right.chars.cycle.reduce(['AAA', 0]) do |node_and_steps, instruction|
      node, steps = node_and_steps
      return steps if node == 'ZZZ'
      [@network[node]["LR".index(instruction)], steps + 1]
    end
  end
end

if defined? DATA
  puts HauntedWasteland.new(DATA.read).steps_required
end
